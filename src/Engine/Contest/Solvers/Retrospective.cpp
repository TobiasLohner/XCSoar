...
Retrospective::PruneCandidates()
{
  ...
  if (erase) {
    if (debug) {
      printf("drop candidate %s\n", it_best->waypoint.name.c_str());
    }
...
Retrospective::UpdateSample(const GeoPoint &aircraft_location)
{
  ...
  if (back.waypoint.id != waypoint->id) {
    ...
    if ((dist_wpwp <= search_range) && (candidate_list.size()>1)) {
      ...
      if (debug) {
	printf("checking previous %s\n", previous->waypoint.name.c_str());
      }
	...
	changed = true;
	if (debug) {
	  printf("Retrospective replaced %s\n", waypoint->name.c_str());
	}
      }
